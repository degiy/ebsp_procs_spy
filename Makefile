...
C_APPS := minimal minimal_legacy minimal_ns bootstrap bootstrap_legacy uprobe kprobe fentry \
       usdt sockfilter tc ksyscall task_iter lsm

CXX_APPS := i

APPS := $(C_APPS) $(CXX_APPS)
...
$(OUTPUT)/%.o: %.cpp $(wildcard %.h) | $(OUTPUT)
	$(call msg,CXX,$@)
	$(Q)$(CXX) $(CFLAGS) -std=c++17 $(INCLUDES) -c $(filter %.cpp,$^) -o $@
...
$(C_APPS): %: $(OUTPUT)/%.o $(LIBBPF_OBJ) | $(OUTPUT)
	$(call msg,BINARY,$@)
	$(Q)$(CC) $(CFLAGS) $^ $(ALL_LDFLAGS) -lelf -lz -o $@

# Build application binary
$(CXX_APPS): %: $(OUTPUT)/%.o $(LIBBPF_OBJ) | $(OUTPUT)
	$(call msg,BINARY++,$@)
	$(Q)$(CXX) $(CFLAGS) $^ $(ALL_LDFLAGS) -lelf -lz -o $@
# 
